...
class Segment {
    ...
    puntInterseccio(segment2) {
        ...
        if (this.esTallen(segment2)) {
            ...
            if (d == 0) {
                ...
                var y = (a1 * c2 - a2 * c1) / d;
                var puntInterseccio = new Punt(x, y);	// aquest punt pertany a les dues rectes
                // comprovem si el punt intersecció pertany als segments
    ...
    // funció interna per determinar si dos segments es tallen
    esTallen(segment2) {
        let s1p1 = this.puntA;
        let s1p2 = this.puntB;
    ...
    // funció interna per determinar si un punt està dins un segment
    contePunt(p1, p2, punt) {
        return (this.valorDinsInterval(p1.x, punt.x, p2.x) || this.valorDinsInterval(p1.y, punt.y, p2.y));
    }
    // funció interna per determinar si un valor està dins un interval
